...
export default (arr) => {
  ...
  const move = (blockCount, from, to) => {
    stacks[to].push(...stacks[from].slice(stacks[from].length - blockCount))
    stacks[from] = stacks[from].slice(0, stacks[from].length - blockCount)
  }
